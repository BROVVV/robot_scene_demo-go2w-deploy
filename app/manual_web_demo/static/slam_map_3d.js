...
(function () {
  ...
  function applySnapshot(snapshot) {
    if (!snapshot || !snapshot.available || !Array.isArray(snapshot.points) || !snapshot.points.length) {
      state.textContent = "等待 plain_slam 实时点云" + (snapshot && snapshot.reason ? " · " + snapshot.reason : "");
    ...
    var age = snapshot.age_seconds == null ? "--" : Number(snapshot.age_seconds).toFixed(1);
    var source = snapshot.source || "pointcloud";
    state.textContent = (snapshot.fresh ? "LIVE" : "STALE") + " · " + source + " · " + age + "s";
    state.className = "slam3d-state" + (snapshot.fresh ? "" : " stale");
    meta.textContent = points.length.toLocaleString() + " 点 · " + (snapshot.frame_id || "pslam_odom") +
      " · voxel " + Number(snapshot.voxel_size_m || 0).toFixed(2) + "m";
    setLight(snapshot.fresh ? "green" : "yellow");
    render();
  }
